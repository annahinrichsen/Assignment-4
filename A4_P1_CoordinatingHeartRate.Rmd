...
#Combine HR data
Conversation_long <- gather(Conversation, Person, HR , HR1:HR2)
#combine resp data
Conversation_long <- gather(Conversation_long, Person_resp, Resp , Resp1:Resp2)
#Get only the digits from new new "person coloumns"
Conversation_long$Person_resp = Conversation_long$Person_resp  %>% str_extract("\\d")
Conversation_long$Person = Conversation_long$Person_resp  %>% str_extract("\\d")
#delete one of them (I think they fit)
Conversation_long$Person_resp = NULL

mean(Conversation_long$HR)
mean(Conversation_long$Resp)


results = stat.desc(Conversation_long$HR)
results[9]
...
m1 = lmerTest::lmer(HR ~ Resp + (1|group), data = exercise4_df, control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
